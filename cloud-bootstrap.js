...
try{
  ...
  const rows=await KMT.progress(u.id);const h=(rows||[]).map(x=>({id:x.worksheet_id,date:x.date,n:x.correct,wrong:x.wrong,notAnswered:x.not_answered,total:x.total,a:x.accuracy,et:x.elapsed,reviewed:x.reviewed,operation:x.operation,range:x.range,userId:x.user_id,userName:x.user_name}));saveHist(h,u.id);renderWeek();renderMonthly();renderYearly();await loadCloudWorksheetStatus(u.id);
