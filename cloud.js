...
(function(){
  ...
  async function finishLogin(email,pin,expectedId){
    ...
    if(!map[0]||(expectedId&&map[0].app_user_id!==expectedId)){
      await logout();throw Error('Security check failed: Auth account is not mapped correctly. Please use Production 3.8.0.');
    }
  ...
  async function parentLogin(email,password){
    ...
    if(!p[0]){await logout();throw Error('Parent profile could not be created. Run the Production 3.8.0 parent SQL migration in Supabase.');}
  ...
  async function ensureParentSession(){
    ...
    if(!p[0]){
      ...
      if(!again[0])throw Error('Parent profile is not available. Run the Production 3.8.0 database migration.');
